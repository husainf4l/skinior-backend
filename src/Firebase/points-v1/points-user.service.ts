...
export class PointsV1Service {
    ...
    async updateBracket(data: { UserUid: string; bracket: number }): Promise<{ success: boolean; message: string }> {
        try {
            await this.vs1Users.doc(data.UserUid).update({
            ...
            return { success: false, message: "Failed to update bracket. Please try again." };
        }
    }
